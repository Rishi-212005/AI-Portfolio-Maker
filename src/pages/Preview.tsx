...
import { Send, Bot, User, Sparkles, Download, ExternalLink, Trash2, Moon, Sun, Palette, Copy, Check, X, Globe, Wand2, GripVertical, RotateCcw, AlertCircle, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Plus, GraduationCap, Briefcase, Award, Trophy, Languages, Save, Code } from "lucide-react";
...
const Preview = () => {
  ...
  const [mobileActiveTab, setMobileActiveTab] = useState<"chat" | "form" | "preview">("preview");
  const [showMobileWarning, setShowMobileWarning] = useState(false);

  useEffect(() => {
    if (window.innerWidth < 1024) {
      const dismissed = sessionStorage.getItem("portgen_mobile_warning_dismissed");
      if (!dismissed) {
  ...
  useEffect(() => {
    if (viewMode !== "visual-edit" && mobileActiveTab === "form") {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex h-screen flex-col pt-16 lg:flex-row">
        {/* Mobile Tab Switcher */}
        <div className="lg:hidden flex border-b border-border/30 bg-background/95 backdrop-blur px-4 py-2 justify-center gap-2 shrink-0 z-40 sticky top-16">
          <button
            type="button"
            onClick={() => setMobileActiveTab("chat")}
            className={`flex-1 max-w-[150px] py-1.5 px-3 text-xs font-semibold rounded-lg text-center transition-all ${
        ...
        <div 
          ...
          } ${mobileActiveTab === "chat" ? "flex w-full" : "hidden lg:flex"}`}
        >
          {/* Header */}
        ...
        {/* Preview Panel */}
        <div className={`flex-1 flex flex-col h-full bg-secondary/10 overflow-hidden relative ${mobileActiveTab !== "chat" ? "flex" : "hidden lg:flex"}`}>
          {/* Collapse/Expand Toggle Tab */}
          <button
            type="button"
            onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-50 hidden lg:flex h-16 w-5 items-center justify-center rounded-r-md border border-l-0 border-border bg-background text-muted-foreground hover:text-foreground shadow-md transition-all duration-200"
          ...
          <div className="flex-1 overflow-hidden flex flex-col lg:flex-row h-full">
            {viewMode === "visual-edit" ? (
              <>
                ...
                <div className={`flex-1 overflow-y-auto scroll-smooth border-r border-border/30 relative ${mobileActiveTab === "preview" ? "block" : "hidden lg:block"}`}>
                ...
                <div className={`w-full lg:w-[420px] shrink-0 bg-background border-t lg:border-t-0 lg:border-l border-border/30 flex flex-col h-full overflow-hidden select-text ${mobileActiveTab === "form" ? "flex" : "hidden lg:flex"}`}>
                  ...
                  <div className="flex-1 overflow-y-auto p-1.5 space-y-2">
                    ...
                    <AccordionItem
                      ...
                      title="Basic Info"
                      icon={User}
                      isOpen={activeEditorSection === "about"}
                      onToggle={() => toggleEditorSection("about")}
                    >
                    ...
                    <AccordionItem
                      ...
                      icon={Code}
                      isOpen={activeEditorSection === "skills"}
                    ...
                    <AccordionItem
                      ...
                      icon={Briefcase}
                      isOpen={activeEditorSection === "projects"}
                      onToggle={() => toggleEditorSection("projects")}
                    >
                      <div className="space-y-3">
                    ...
                    <AccordionItem
                      ...
                      icon={Briefcase}
                      isOpen={activeEditorSection === "experience"}
                    ...
                    <AccordionItem
                      ...
                      icon={GraduationCap}
                      isOpen={activeEditorSection === "education"}
                    ...
                    <AccordionItem
                      ...
                      icon={Award}
                      isOpen={activeEditorSection === "certifications"}
                      onToggle={() => toggleEditorSection("certifications")}
                    >
                      <div className="space-y-3">
                    ...
                    <AccordionItem
                      ...
                      title="Achievements"
                      icon={Trophy}
                      isOpen={activeEditorSection === "achievements"}
                      onToggle={() => toggleEditorSection("achievements")}
                    >
                      <div className="space-y-3">
                    ...
                    <AccordionItem
                      ...
                      title="Languages"
                      icon={Languages}
                      isOpen={activeEditorSection === "languages"}
                      onToggle={() => toggleEditorSection("languages")}
                    >
                      <div className="space-y-3">
        ...
        {showMobileWarning && (
          ...
          >
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 20 }}
              className="glass-card w-full max-w-md rounded-2xl p-6 text-center border border-border/40 space-y-6"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-bold text-foreground">Desktop Experience Recommended</h3>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  The AI Portfolio Editor is designed and optimized for larger desktop screens. 
                  For the best customization and editing experience, we recommend using a desktop or laptop computer.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2.5 pt-2">
                <Button 
                  onClick={() => {
                    sessionStorage.setItem("portgen_mobile_warning_dismissed", "true");
                    setShowMobileWarning(false);
                  }}
                  className="w-full bg-primary text-primary-foreground hover:bg-primary/90 text-xs font-semibold py-2.5"
                >
                  Continue on Mobile
                </Button>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link to="/login" className="w-full">
                    <Button 
                      variant="outline" 
                      className="w-full border-border/50 text-xs font-semibold py-2.5"
                    >
                      Return to Login
                    </Button>
                  </Link>
